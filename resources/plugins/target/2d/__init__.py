...
_params = dict(
            start=dict(),
            stop=dict(),
            load=dict(emiss=1.0, coverage=0.2),
            run=dict(),
            get=dict(),
    )
...
class Plugin(targ.BaseTargPlugin):
    ...
    def stop(self):
        pass
    def load(self, emiss=1.0, coverage=0.2):
        self.background = targ.get_background()
    ...
    def run(self,):
        ...
        integrand = lambda az, zen, ii: self.background.get(zen,az)[ii]*cos(d2r(zen))*sin(d2r(zen))
    ...
    def get(self, **kwargs):
        ...
        target = saig.dummySAIG(lambda zen, az: self.emiss*self.integrated_rad/pi)
